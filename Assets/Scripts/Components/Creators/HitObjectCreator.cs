...
public class HitObjectCreator : MonoBehaviour
{
    ...
    private void WaitToSpawnEnemy()
    {
        ...
        {
            if (GameState.GetSpeed(_offset) * (-AudioPlayer.Instance.GetTrueAudioTime() + Assets.Instance.Level.Enemies[i].SpawnTime) + 5.6 * Assets.Instance.SavedSettings.ElementsSize > 1100) return;
